...
class StyledConv(nn.Module):
    def __init__(
        ...
        self.dense =  EqualLinear(embedding_dim,out_channel,bias_init = 0,activation="fused_lrelu")
...
class Generator(nn.Module):
    def __init__(
        ...
        for i in range(3, self.log_size + 1):
            ...
            self.convs.append(
                StyledConv(
                    out_channel, out_channel, 3, style_dim,embedding_dim, blur_kernel=blur_kernel
                )
            )
...
class ResBlock_cond(nn.Module):
    def __init__(self, in_channel, out_channel,t_emb_dim, blur_kernel=[1, 3, 3, 1], downsample=True, skip_gain=1.0,residual=True):
        super().__init__()
...
class TimestepEmbedding(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, output_dim, act=nn.LeakyReLU(0.2)):
        ...
        self.main = nn.Sequential(
            EqualLinear(embedding_dim, hidden_dim,bias_init = 0, activation='fused_lrelu'),
...
class StyleGAN2Discriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, t_emb_dim=4*64, no_antialias=False, size=None, opt=None):
        super().__init__()
        self.opt = opt
        ...
        for i in range(log_size, final_res_log2, -1):
            ...
            self.convs.append(ResBlock_cond(in_channel, out_channel, t_emb_dim,blur_kernel,residual=False))
    ...
    def forward(self, input,t, get_minibatch_features=False):
        ...
        if "patch" in self.opt.netD and self.opt.D_patch_size is not None:
            ...
            input_x = input_x[:, :, y:y + self.opt.D_patch_size, x:x + self.opt.D_patch_size]
        out = input
...
class StyleGAN2Decoder(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, opt=None):
        ...
        for i in range(n_blocks // 2):
            ...
            conv_init.append(StyledConv(n_channel, n_channel, 3, upsample=False, blur_kernel=blur_kernel, inject_noise=inject_noise,style_dim=self.opt.style_dim,embedding_dim=4*ngf))
        for i in range(num_downsampling):
            in_channel = channels[cur_res]
            out_channel = channels[cur_res * 2]
            inject_noise = "small" not in self.opt.netG
            conv_init.append(
                StyledConv(in_channel, out_channel, 3, upsample=True, blur_kernel=blur_kernel, inject_noise=inject_noise,style_dim=self.opt.style_dim,embedding_dim=4*ngf)
            )
            cur_res = cur_res * 2
...
class StyleGAN2Generator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, opt=None):
        ...
        for i in range(self.opt.n_mlp):
            layers.append(
                EqualLinear(
                    self.opt.style_dim, self.opt.style_dim, lr_mul=self.opt.lr * 0.01, activation="fused_lrelu"
                )
            )
        ...
        layers_time.append(EqualLinear(ngf* 4, ngf * 4,activation="fused_lrelu", lr_mul=self.opt.lr * 0.01))
